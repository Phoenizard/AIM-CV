...
class ConstraintSet : public ArmorDetectionBase {
 ...
 private:
  ...
  public:
  void generate_proposals(const ncnn::Mat& pred, int stride, const ncnn::Mat& in_pad, float prob_threshold, std::vector<ArmorInfo>& armorInfos, int wpad, int hpad, float scale);
  ArmorInfo nonaDetect(cv::Mat src_img_);
