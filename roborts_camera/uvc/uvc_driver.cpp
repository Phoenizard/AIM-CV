...
void UVCDriver::StartReadCamera(cv::Mat &img) {
  if(!camera_initialized_){
   ...
   // 
    ...
    rs2::frame RGBData = data.get_color_frame().apply_filter(color_map);
   

    const int w = RGBData.as<rs2::video_frame>().get_width();
    const int h = RGBData.as<rs2::video_frame>().get_height();

    cv::Mat image(cv::Size(w, h), CV_8UC3, (void*)RGBData.get_data(), cv::Mat::AUTO_STEP);
